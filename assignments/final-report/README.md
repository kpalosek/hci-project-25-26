...
Upon landing on **home page**, the first thing user can see is a large **hero section** with **search bar** used to fulfill the **main purpose of the application**, allow users to search for a desired destination.

<img src="./images/visibility-02.png" alt="Visibility Image" width="800">
...
While some features are locked for users that are not signed in, those features are still **visible to all users** to improve **visibility** principle. If an user that is not signed in tries to access one of those features, a **sign in prompt** will pop up. Examples are heart button to add a guide to saved guides, and post an update button that allows registered user to post updates.
...
While not signed in, navigation bar displays Sign In button. When user signs in, navigation bar **changes state** to display user's username and Sign Out button, allowing user to clearly see that the action has been done.
...
Feedback is also given to user when **signing in / registering** to let him know what's missing or incorrect, for example:
...
While searching for guides using the Explore section, at each level user can see the **whole path** that led him to where he currently is and it enables him to directly jump to a desired level (world / continent / country / city).
...
**Heart icon** displayed on each guide card also works as an example of mapping as it indicates the action that will be performed upon clicking (adding guide to favorite / liked / saved) and it is obvious to which guide it applies by being a part of the card.
...
**Hover effect** with corresponding **label** indicate that cards are clickable.

<img src="./images/affordance-04.png" alt="Consistency Image" height="300"> <img src="./images/affordance-05.png" alt="Consistency Image" height="300">


## Performance analysis

Results of **PageSpeed Insights** performance analysis:

### Desktop:
<img src="./images/performance-desktop.png" alt="Performance desktop" height="800">

### Mobile:
<img src="./images/performance-mobile.png" alt="Performance mobile" height="800">