  ...
  try {
    const airportUpdates = await db
      .select({
        ...
        // Dohvaćamo i ime korisnika koji je objavio (iz tablice user)
        userName: user.name, 
      })
      .from(update)
      .leftJoin(user, eq(update.userId, user.id)) // Spajamo tablice preko userId
      .where(eq(update.airportCode, airportCode))
      .orderBy(desc(update.createdAt)); // Najnoviji prvi
